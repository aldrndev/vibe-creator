...
**Biome Clean Definition:**

- "Biome clean" means **0 errors AND 0 warnings**
- `biome check` MUST produce zero diagnostics before code is considered deliverable
- During active development, warnings are tolerated temporarily — but MUST be resolved before commit/PR/delivery
- AI agents MUST NOT mark Biome as "clean" if any warnings remain
...
- No nested ternary operators (error) — use early returns or extracted variables
- No negated conditions with else clause (warn) — flip the condition for readability
- No direct mutation of React props (error)
- No excessive cognitive complexity (error, max 15)
- No `forEach` (error) — use `for...of` or functional methods (`.map`, `.filter`, `.reduce`)
- Use optional chaining (error) — `obj?.prop` instead of `obj && obj.prop`
- Use `Number.parseInt()` / `Number.isNaN()` (error) — not global `parseInt()` / `isNaN()`
- Throw only `Error` objects (error) — `throw new Error()`, never `throw "string"` or `Promise.reject("string")`
- `.sort()` / `.toSorted()` MUST use a compare function — default sort is lexicographic
- Do NOT use return value of array-mutating methods (`.sort()`, `.reverse()`) as if it were a new array — they mutate in place
