...
- [ ] Biome clean — `biome check --changed` produces 0 errors AND 0 warnings on modified files
- [ ] Complexity — no function exceeds cognitive complexity of 15
...
- Write nested ternary operators — extract to variables or use early returns
- Write negated conditions with else clauses — flip the condition
- Ignore Biome warnings — all warnings MUST be resolved before delivering code
- Throw string literals — always `throw new Error()` or `Promise.reject(new Error())`
- Use `.sort()` without a compare function — default sort is lexicographic, not numeric
- Use return value of `.sort()` / `.reverse()` as a new array — they mutate in place, use `.toSorted()` / `.toReversed()` for immutable versions
