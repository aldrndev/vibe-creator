...
describe("director handlers", () => {
  ...
  describe("analysis handler idempotency", () => {
    it("should skip if analysis already completed", async () => {
      ...
      const { processAnalysisJob } = await import(
        "@/modules/director/handlers/analysis.handler"
      );
  ...
  describe("export handler", () => {
    it("should throw if session not found", async () => {
      ...
      const { processExportJob } = await import(
        "@/modules/director/handlers/export.handler"
      );
      ...
      await expect(processExportJob(mockJob)).rejects.toThrow(
        "Session or export job not found"
      );
    });
  });
});
