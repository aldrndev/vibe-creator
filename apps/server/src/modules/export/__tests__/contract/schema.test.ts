...
describe("export.schema contracts", () => {
  ...
  describe("EDITOR_LIMITS", () => {
    ...
    it("should have clip limits", () => {
      ...
      expect(EDITOR_LIMITS.MIN_CLIP_DURATION_MS).toBeLessThan(
        EDITOR_LIMITS.MAX_CLIP_DURATION_MS
      );
    });
  });
  ...
  describe("timelineClipSchema", () => {
    ...
    it("should reject endMs <= startMs", () => {
      expect(() =>
        timelineClipSchema.parse({ ...validClip, endMs: 0 })
      ).toThrow();
    });
  ...
  describe("editorProjectDTOSchema", () => {
    ...
    it("should accept minimal valid project", () => {
      expect(() =>
        editorProjectDTOSchema.parse(minimalValidProject)
      ).not.toThrow();
    });
  ...
  describe("QUALITY_PRESETS", () => {
    it("should have increasing bitrates for quality levels", () => {
      const bitrates = [
        parseInt(QUALITY_PRESETS.low.videoBitrate),
        parseInt(QUALITY_PRESETS.medium.videoBitrate),
        parseInt(QUALITY_PRESETS.high.videoBitrate),
        parseInt(QUALITY_PRESETS.lossless.videoBitrate),
      ];
