...
export const ffmpegProcessorCLI = {
  ...
  async trim(options: TrimOptions): Promise<void> {
    const { inputPath, outputPath, startTime, endTime, totalDuration, signal, onProgress } = options;
  ...
  async addWatermark(options: WatermarkOptions): Promise<void> {
    ...
    const args = [
      ...
      '-progress', 'pipe:1',
      '-i', validInput,
      '-vf', `drawtext=text='${escapedText}':fontsize=24:fontcolor=white@0.5:x=w-tw-20:y=h-th-20`,
      '-c:v', 'libx264',
      '-c:a', 'copy',
      '-preset', 'fast',
      validOutput,
  ...
  async applyEffects(options: ApplyEffectsOptions): Promise<void> {
    ...
    logger.info(
      { inputPath: validInput, transforms, effects },
