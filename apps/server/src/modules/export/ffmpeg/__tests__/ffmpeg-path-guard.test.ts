...
import { mkdirSync, writeFileSync, rmdirSync, unlinkSync, symlinkSync } from 'fs';
...
describe('ffmpeg-path-guard', () => {
  beforeAll(() => {
    // Create test directories FIRST (before env vars)
    mkdirSync(TEST_UPLOADS, { recursive: true });
    mkdirSync(TEST_TEMP, { recursive: true });
    mkdirSync(TEST_EXPORTS, { recursive: true });
    
    // Set env vars AFTER creating directories
    process.env.MEDIA_INPUT_DIR = TEST_UPLOADS;
    process.env.MEDIA_TEMP_DIR = TEST_TEMP;
    process.env.MEDIA_OUTPUT_DIR = TEST_EXPORTS;
  ...
  describe('validateInputPath', () => {
    ...
    it('should reject path traversal attempts', () => {
      expect(() => validateInputPath('../../../etc/passwd'))
        .toThrow('Path traversal not allowed');
    });

    it('should reject URL inputs (SSRF)', () => {
      expect(() => validateInputPath('http://evil.com/video.mp4'))
        .toThrow('URL inputs not allowed');
      
      expect(() => validateInputPath('https://evil.com/video.mp4'))
        .toThrow('URL inputs not allowed');
      
      expect(() => validateInputPath('file:///etc/passwd'))
        .toThrow('URL inputs not allowed');
      
      expect(() => validateInputPath('ftp://server/file.mp4'))
        .toThrow('URL inputs not allowed');
    });

    it('should reject null bytes (poison attack)', () => {
      expect(() => validateInputPath('video.mp4\x00.txt'))
        .toThrow('Null bytes not allowed');
    });

    it('should reject non-existent files', () => {
      expect(() => validateInputPath(join(TEST_UPLOADS, 'nonexistent.mp4')))
        .toThrow('Input file not found');
    });

    it('should reject paths outside allowed directories', () => {
      expect(() => validateInputPath('/etc/passwd'))
        .toThrow(); // Either "not found" or "outside allowed"
    });
  });
  ...
  describe('validateOutputPath', () => {
    ...
    it('should reject path traversal in output', () => {
      expect(() => validateOutputPath('../../../tmp/malicious.mp4'))
        .toThrow('Path traversal not allowed');
    });

    it('should reject URL outputs', () => {
      expect(() => validateOutputPath('http://evil.com/upload'))
        .toThrow('URL outputs not allowed');
    });

    it('should reject null bytes in output path', () => {
      expect(() => validateOutputPath('output.mp4\x00.txt'))
        .toThrow('Null bytes not allowed');
    });
  });
