...
describe("streamService", () => {
  ...
  describe("stopStream", () => {
    it("should stop active stream", async () => {
      ...
      mockPrisma.streamSession.update.mockResolvedValue({
        ...stream,
        status: "ENDED",
      });
    ...
    it("should throw when stream not found", async () => {
      ...
      // Act & Assert
      await expect(
        streamService.stopStream("nonexistent", "user-123")
  ...
  describe("getStreamStatus", () => {
    it("should return stream status", async () => {
      ...
      // Act
      const result = await streamService.getStreamStatus(
        "stream-123",
        "user-123"
      );
    ...
    it("should throw when stream not found", async () => {
      ...
      // Act & Assert
      await expect(
        streamService.getStreamStatus("nonexistent", "user-123")
      ).rejects.toThrow("Stream not found");
    });
  });
