    ...
    try {
      ...
      if (expiredStreams.length > 0) {
        ...
        for (const s of expiredStreams) {
          ...
          await streamService
            .stopStream(s.id, s.userId, "AUTO_STOP")
            .catch((err) => {
              logger.error(
                { err, streamId: s.id },
...
export const streamService = {
  ...
  async startStream(input: StartStreamInput): Promise<{ streamId: string }> {
    ...
    if (activeUserStreams.has(userId)) {
      logger.info(
        { userId },
        "Stopping existing stream to start new one (Concurrency Check)"
      );
    ...
    const args = [
      ...
      inputPath,
      "-vf",
      `scale=${settings.w}:${settings.h}`, // Scale to Quality
      "-c:v",
      "libx264",
      "-preset",
      "veryfast",
    ...
    logger.info(
      { streamId: stream.id, autoStopAt },
      "Stream started with auto-stop deadline"
    );
    ...
    process.stderr.on("data", (data) => {
      ...
      if (
        ...
        logger.debug(
          { streamId: stream.id, data: logStr },
          "ffmpeg stream stderr"
        );
      }
    });
