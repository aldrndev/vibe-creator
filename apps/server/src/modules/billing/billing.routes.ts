...
export const billingRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.get('/packages', async (_request, reply) => {
      return reply.send({
          success: true,
   ...
   */
  fastify.post<{ Body: { packageId: string } }>('/topup', async (request, reply) => {
      const user = request.user;
      if (!user) return reply.status(401).send({ error: 'Unauthorized' });
      ...
      try {
          ...
          request.log.error(e);
          return reply.status(500).send({ error: e instanceof Error ? e.message : 'Topup creation failed' });
      }
  });
