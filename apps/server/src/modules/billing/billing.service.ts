...
// Constants
const XENDIT_SECRET_KEY = process.env.XENDIT_SECRET_KEY || 'xnd_development_...'; // Fallback for dev
...
export const billingService = {
  ...
  async createTopupInvoice(userId: string, packageId: string) {
      ...
      const invoice = await Invoice.createInvoice({
          data: {
              ...
              successRedirectUrl: `${process.env.APP_URL || 'http://localhost:5173'}/tools/live-stream`,
              failureRedirectUrl: `${process.env.APP_URL || 'http://localhost:5173'}/tools/live-stream?status=failed`,
              currency: 'IDR'
          }
      });
  ...
  async handleWebhook(event: any) {
      const { status, external_id } = event;
      ...
      try {
        await prisma.paymentEvent.create({
            data: {
                provider: 'XENDIT',
                providerEventId: event.id || `evt_${Date.now()}_${Math.random()}`, // Xendit webhook usually has an ID if using newer API, or use invoice update ID
      ...
      // Process Purchase
      const purchase = await prisma.streamTopupPurchase.findUnique({ where: { id: purchaseId } });
      if (!purchase) {
  ...
  async applyTopUp(purchaseId: string) {
      ...
      return await prisma.$transaction(async (tx) => {
          ...
          const now = new Date();
           ...
           logger.info({ userId: purchase!.userId, minutes: purchase!.minutes }, 'Top-up applied to cycle');
      });
  }
};
