...
describe("auth session", () => {
  ...
  describe("constants", () => {
    it("should have access token duration of 60 minutes", () => {
      expect(ACCESS_TOKEN_DURATION_MINUTES).toBe(60);
    });

    it("should have refresh token duration of 30 days", () => {
      expect(REFRESH_TOKEN_DURATION_DAYS).toBe(30);
    });
  });
  ...
  describe("createSession", () => {
    ...
    it("should set correct access token expiration (60 minutes)", async () => {
      ...
      const expectedExpiry = new Date("2024-01-01T01:00:00.000Z");
      expect(result.accessExpiresAt).toEqual(expectedExpiry);
    });

    it("should set correct refresh token expiration (30 days)", async () => {
      const result = await createSession(
        TEST_USER_ID,
        TEST_USER_AGENT,
        TEST_IP
      ...
      const expectedExpiry = new Date("2024-01-31T00:00:00.000Z");
