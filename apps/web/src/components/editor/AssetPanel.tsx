...
export function AssetPanel({ className }: AssetPanelProps) {
  ...
  return (
    <div
      className={clsx(
        "w-64 border-r border-border flex flex-col bg-background/50 flex-shrink-0",
        className
      )}
    >
        ...
        {assets.length === 0 ? (
          ...
          <div className="space-y-2">
            {assets.map((asset) => (
              <div
                key={asset.id}
                className="group relative flex items-center gap-3 p-2 rounded-lg hover:bg-muted transition-colors cursor-pointer border border-transparent hover:border-border"
              >
                <div className="w-10 h-10 rounded bg-muted flex items-center justify-center text-muted-foreground flex-shrink-0 overflow-hidden">
                  {getIcon(asset.type)}
                </div>
