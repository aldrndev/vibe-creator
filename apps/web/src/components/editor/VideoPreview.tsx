      ...
      <div className="absolute inset-0 pointer-events-none border-2 border-primary/50 rounded-lg animate-in fade-in zoom-in duration-200">
        {handles.map(({ pos, style, cursor }) => (
          <button
            type="button"
            key={pos}
            aria-label={`Resize text overlay from ${pos}`}
        ...
        <button
          type="button"
          aria-label="Rotate text overlay"
...
export function VideoPreview() {
  ...
  const { timeline, currentTimeMs, isPlaying, projectSettings } = useEditorStore();
  ...
  useEffect(() => {
    ...
    const videoTimeSec = (currentTimeMs - activeClip.startMs + activeClip.trimStartMs) / 1000;
    if (Math.abs(video.currentTime - videoTimeSec) > 0.1) video.currentTime = videoTimeSec;
