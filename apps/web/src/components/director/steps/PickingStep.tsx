import { CheckCircle2, FileVideo, Play } from 'lucide-react';
...
function CandidateCard({
  ...
  const previewFileName = clip.previewStorageKey?.split('/').pop();
  const previewUrl = useAuthenticatedObjectUrl(
    previewFileName
      ? `/api/v1/director/sessions/${activeSession.id}/previews/${previewFileName}`
      : null,
  );
  const assetStreamUrl = useAuthenticatedObjectUrl(
    isPlaying ? `/api/v1/director/sessions/${activeSession.id}/asset/stream` : null,
  );
          ...
          <div className="relative w-full h-full">
            <video
              ...
              onTimeUpdate={(e) => {
                ...
                const progressBar = document.getElementById(`progress-${clip.id}`);
                if (progressBar) {
              ...
              onLoadedMetadata={(e) => {
                ...
                video.play().catch((error) => logger.error('Video play failed', error));
              }}
              onClick={(e) => {
                e.stopPropagation();
                onStopPlaying();
