...
export function LiveStreamSettings({
  ...
  const quotaHeadline = !quota
    ...
    : isQuotaUnlimited
      ...
      : isQuotaEmpty
        ? 'Quota habis'
        : `${quotaRemainingValue ?? 0} menit tersisa`;
  const quotaDescription = !quota
    ? 'Quota streaming akan muncul di sini.'
    : isQuotaUnlimited
      ? 'Akses admin tanpa batas streaming.'
      : `${quotaUsed} dari ${quotaTotal ?? 0} menit terpakai bulan ini`;
