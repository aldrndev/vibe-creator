import { AlertTriangle, CheckCircle2, Download, Loader2, Pencil, Repeat2 } from 'lucide-react';
import { Button, Dialog, DialogContent, Progress } from '@/components/ui';
import type { LoopRenderPhase, LoopRenderResult } from '@/hooks/useLoopCreator';

interface LoopRenderDialogProps {
  readonly open: boolean;
  readonly phase: LoopRenderPhase;
  readonly progress: number;
  readonly error: string | null;
  readonly notice: string | null;
  readonly result: LoopRenderResult | null;
...
export function LoopRenderDialog({
  open,
  phase,
  progress,
  error,
            ...
            <>
              ...
              <div className="rounded-xl border border-border/45 bg-muted/10 p-4">
                ...
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  {result.urlExpiresAt
                    ? `Download tersedia sampai ${new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(result.urlExpiresAt))}.`
...
function getPhaseLabel(phase: LoopRenderPhase): string {
  return phase === 'queued' ? 'Menunggu antrian render...' : 'Memperpanjang video kamu...';
}
