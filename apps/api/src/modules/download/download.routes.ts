...
export const downloadRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/request", async (request, reply) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Download request failed";
      return reply.status(400).send({
        success: false,
  ...
  fastify.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const status = await downloadService.getJobStatus(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return reply
          ...
          .header(
            "Content-Disposition",
            `attachment; filename="${status.title || "video"}.mp4"`
          )
  ...
  fastify.get("/history", async (request, reply) => {
    ...
    const result = await downloadService.getHistory(
      user.id,
      limit,
      query.cursor
    );
