...
describe('auth.schema contracts', () => {
  describe('registerSchema', () => {
    ...
    it('should reject invalid email', () => {
      expect(() => registerSchema.parse({ ...validPayload, email: 'invalid' })).toThrow(
        'Email tidak valid',
      );
    });

    it('should reject short password', () => {
      expect(() => registerSchema.parse({ ...validPayload, password: 'short' })).toThrow(
        'Password minimal 8 karakter',
      );
    });

    it('should reject short name', () => {
      expect(() => registerSchema.parse({ ...validPayload, name: 'J' })).toThrow(
        'Nama minimal 2 karakter',
      );
    });

    it('should reject missing captcha', () => {
      expect(() => registerSchema.parse({ ...validPayload, turnstileToken: '' })).toThrow(
        'Captcha diperlukan',
      );
    });
  ...
  describe('loginSchema', () => {
    ...
    it('should reject invalid email', () => {
      expect(() => loginSchema.parse({ ...validPayload, email: 'not-an-email' })).toThrow(
        'Email tidak valid',
      );
    });

    it('should reject empty password', () => {
      expect(() => loginSchema.parse({ ...validPayload, password: '' })).toThrow(
        'Password diperlukan',
      );
    });
    ...
    it('should accept any password length for login', () => {
      ...
      expect(() => loginSchema.parse({ ...validPayload, password: 'x' })).not.toThrow();
    });

    it('should reject missing captcha', () => {
      expect(() => loginSchema.parse({ ...validPayload, turnstileToken: '' })).toThrow(
        'Captcha diperlukan',
      );
    });
  });
