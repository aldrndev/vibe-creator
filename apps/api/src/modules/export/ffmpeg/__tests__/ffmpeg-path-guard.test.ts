...
const { validateInputPath, validateOutputPath, createJobTempDir, getAllowlistedDirs } =
...
describe('ffmpeg-path-guard', () => {
  ...
  describe('validateInputPath', () => {
    ...
    it('should reject path traversal attempts', () => {
      expect(() => validateInputPath('../../../etc/passwd')).toThrow('Path traversal not allowed');
    });
    ...
    it('should reject URL inputs (SSRF)', () => {
      ...
      expect(() => validateInputPath('file:///etc/passwd')).toThrow('URL inputs not allowed');

      expect(() => validateInputPath('ftp://server/file.mp4')).toThrow('URL inputs not allowed');
    });

    it('should reject null bytes (poison attack)', () => {
      expect(() => validateInputPath('video.mp4\x00.txt')).toThrow('Null bytes not allowed');
    });

    it('should reject non-existent files', () => {
      expect(() => validateInputPath(join(TEST_UPLOADS, 'nonexistent.mp4'))).toThrow(
  ...
  describe('validateOutputPath', () => {
    ...
    it('should reject URL outputs', () => {
      expect(() => validateOutputPath('http://evil.com/upload')).toThrow('URL outputs not allowed');
    });

    it('should reject null bytes in output path', () => {
      expect(() => validateOutputPath('output.mp4\x00.txt')).toThrow('Null bytes not allowed');
    });
  });
